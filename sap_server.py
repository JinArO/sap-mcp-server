...
class SAPConfig:
    SERVICES = {
        ...
        "COMMIT": {
            "url": "https://vhivcqasci.sap.inventec.com:44300/sap/bc/srt/rfc/sap/zws_bapi_salesorder_create/100/zws_bapi_salesorder_create_sev/zws_bapi_salesorder_create_binding",
            "action": '"urn:sap-com:document:sap:rfc:functions:BAPI_TRANSACTION_COMMIT:BAPI_TRANSACTION_COMMITRequest"'
        }
    }
...
# Helper Functions
# ==============================================================================
def _commit_transaction() -> str:
    """Execute BAPI_TRANSACTION_COMMIT to persist SAP changes"""
    commit_body = '<urn:BAPI_TRANSACTION_COMMIT><WAIT>X</WAIT></urn:BAPI_TRANSACTION_COMMIT>'
    try:
        return SAPClient("COMMIT").post_soap(commit_body)
    except Exception as e:
...
def create_sales_order(
    ...
    result = SAPClient("SO").post_soap(xml_body)
    commit_result = _commit_transaction()
...
def create_sto_po(
    ...
    result = SAPClient("STO").post_soap(xml_body)
    commit_result = _commit_transaction()
    return f"{result}\n\n[COMMIT]: {commit_result}"
...
def create_outbound_delivery(
    ...
    result = SAPClient("DN").post_soap(xml_body)
    commit_result = _commit_transaction()
    return f"{result}\n\n[COMMIT]: {commit_result}"
...
def maintain_info_record(
    ...
    result = SAPClient("INF").post_soap(xml_body)
    commit_result = _commit_transaction()
    return f"{result}\n\n[COMMIT]: {commit_result}"
...
def maintain_sales_view(
    ...
    result = SAPClient("MAT").post_soap(xml_body)
    commit_result = _commit_transaction()
    return f"{result}\n\n[COMMIT]: {commit_result}"
...
def maintain_source_list(
    ...
    result = SAPClient("SRC").post_soap(xml_body)
    commit_result = _commit_transaction()
    return f"{result}\n\n[COMMIT]: {commit_result}"
